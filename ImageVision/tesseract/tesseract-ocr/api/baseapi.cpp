...
std::string WideToMutiByte(const std::wstring&_src)
{
...
std::wstring MultiByteToWide(const std::string&_src)
{
...
void TessBaseAPI::GetAvailableLanguagesAsVector(
  ...
  if (tesseract_ != NULL) {
    ...
    BOOL result = TRUE;
	#ifdef _UNICODE
	std::string tmp = pattern.string();
	std::wstring procNew = MultiByteToWide(tmp);
	std::wstring pathful = data.cFileName;
	std::string fulpath = WideToMutiByte(pathful);

	HANDLE handle = FindFirstFile(procNew.c_str(), &data);
	if (handle != INVALID_HANDLE_VALUE) {
		for (; result; result = FindNextFile(handle, &data)) {
			_splitpath(fulpath.c_str(), NULL, NULL, fname, NULL);		
