...
if (g.ini.m_imageDealMode)
{
		...
		cv::Mat matOcr;
		m_ResultImg(rt[0]).copyTo(matOcr);
		src = matOcr.clone();
		return 1.0;
		IdentifyCharater(matOcr);
...
char* opencv::IdentifyCharater(cv::Mat &src, char* language)
{
	m_ocr.SetImage(src.data, src.cols, src.rows, 1, src.step);
	char* text = m_ocr.GetUTF8Text();
	CString str = (CString)CStringA(text);
	AfxMessageBox(str);
	return text;
}
...
, mat_data3(NULL)
{
	...
	m_ocr.SetPageSegMode(tesseract::PSM_SINGLE_BLOCK);
}
